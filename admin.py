...
def setup_initial_admin():
    """Setup initial admin user - Store setup will be done via web interface"""
    ...
    with transaction.atomic():
        admin_exists = User.objects.filter(username=DEFAULT_ADMIN_USERNAME).exists()
        ...
        if admin_exists:
            ...
            User.objects.create_user(
                ...
                is_superuser=True,
                is_active=True,
                has_completed_setup=False  # Will complete via web interface
            )
    ...
    print("   2. Complete store setup through the web interface")
    print("   3. Change your password after first login")
    print("   4. Add additional users and stores as needed")
    print("="*70 + "\n")
